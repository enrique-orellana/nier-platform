  ...
  return (
    ...
    >
      ...
      <div className="flex-1 p-4 flex flex-col bg-[#121214] min-w-0">
        <div className="mb-3">
          <h3
            className="text-sm font-bold text-white leading-tight line-clamp-2 mb-2 break-words"
            title={clip.title}
          >
            {clip.title}
          </h3>
          <div className="flex flex-wrap gap-2 text-[10px] text-zinc-500 font-mono">
            ...
            <span
              className="bg-white/5 px-1.5 py-0.5 rounded border border-white/5 truncate max-w-[150px]"
              title={clip.job_id}
            >
              ID: {clip.job_id.substring(0, 8)}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-2 flex-1 overflow-y-auto custom-scrollbar max-h-[150px] pr-1 mb-3">
          ...
          <div className="bg-black/20 rounded-lg p-2 border border-white/5 relative group/item">
            ...
            <p className="text-xs text-zinc-300 select-all line-clamp-2 hover:line-clamp-none transition-all">
              {clip.title}
            </p>
            <button
          ...
          <div className="bg-black/20 rounded-lg p-2 border border-white/5 relative group/item">
            ...
            <button
              onClick={() =>
                handleCopy(clip.tiktok_desc || clip.insta_desc, "caption")
              }
              className="absolute top-2 right-2 p-1 text-zinc-500 hover:text-white transition-colors opacity-0 group-hover/item:opacity-100"
              title="Copy Caption"
            >
