...
describe("HighlightsTab", () => {
  ...
  it("submits one selected MinIO video with the target and AI headers", async () => {
    ...
    expect(request.headers).toEqual(
      expect.objectContaining({ "X-AI-Provider": "ollama" }),
    );
  ...
  it("stops an active job and displays a completed result", async () => {
        ...
        projects: [
          {
            ...
            name: "Source",
            status: "processing",
            job: { id: "job-1", status: "processing", logs: ["Analyzing"] },
          },
        ],
      }),
    });
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/highlights/projects/project-1/cancel",
      expect.objectContaining({ method: "POST" }),
    );
  });
  ...
  it("renders completed video and manifest links from persisted results", async () => {
        ...
        projects: [
          {
            ...
            job: {
              ...
              logs: ["Ready"],
              result: {
                video_url: "/videos/job-2/highlights.mp4",
                manifest_url: "/videos/job-2/manifest.json",
              },
            },
          },
        ],
    ...
    render(<HighlightsTab getAiHeaders={getAiHeaders} aiProvider="ollama" />);
    expect(
      await screen.findByRole("link", { name: /download highlights/i }),
    ).toHaveAttribute("href", "/videos/job-2/highlights.mp4");
    expect(
      screen.getByRole("link", { name: /view manifest/i }),
    ).toHaveAttribute("href", "/videos/job-2/manifest.json");
  });
});
