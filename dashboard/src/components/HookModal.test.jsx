...
describe('HookModal Streamer Stack configuration', () => {
    it('includes clip layout metadata in the Remotion hook config', () => {
        ...
        expect(screen.getByTestId('remotion-preview')).toHaveAttribute('data-layout', 'streamer_stack');
        expect(screen.getByTestId('remotion-preview')).toHaveAttribute('data-facecam-size', 'large');
        ...
        expect(hookConfigSchema.parse(onGenerate.mock.calls[0][0].remotion)).toMatchObject({
            layoutFormat: 'streamer_stack',
            facecamSize: 'large',
        });
    });
