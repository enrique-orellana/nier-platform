    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                const response = await fetch(getApiUrl(`/api/minio/objects?${query.toString()}`), { signal: controller.signal });
                const payload = await response.json().catch(() => ({}));
                if (!response.ok) throw new Error(payload.detail || 'Could not load MinIO objects.');
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-2 text-sm text-zinc-300">
                ...
                <span>MinIO source bucket: <strong className="text-white">{bucket}</strong></span>
            </div>
            ...
            {loading && <p className="text-sm text-zinc-500">Loading MinIO objects...</p>}

            {!loading && error && (
                <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-200">
                    <p>Could not load MinIO objects: {error}</p>
                    <button type="button" onClick={() => setReloadToken((value) => value + 1)} className="mt-3 inline-flex items-center gap-2 text-red-100 underline">
                        ...
                        return (
                            ...
                            >
                                <div className="flex items-start justify-between gap-3">
                                    <span className="min-w-0 truncate font-medium text-white">{object.name || object.key}</span>
                                    {isSelected && <Check size={17} className="shrink-0 text-primary" />}
                                </div>
                                <div className="mt-1 truncate text-xs text-zinc-500">{object.key}</div>
            ...
            {selected && (
                <div className="rounded-xl border border-primary/30 bg-primary/5 p-3 text-sm text-zinc-300">
                    Selected: <span className="font-medium text-white">{selected.key}</span>
                </div>
            )}
        </div>
    );
