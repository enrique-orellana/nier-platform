...
describe('MinioObjectPicker', () => {
  ...
  it('loads and selects an object from the source bucket', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /select source\.bin/i }));
  ...
  it('shows an error and retry action when MinIO is unavailable', async () => {
    ...
    expect(await screen.findByText(/could not load minio objects/i)).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /retry/i })).toBeInTheDocument();
  });

  it('reloads objects when the user searches', async () => {
    fetch
      .mockResolvedValueOnce({ ok: true, json: async () => ({ bucket: 'youtube-downloads', objects: [] }) })
      .mockResolvedValueOnce({ ok: true, json: async () => ({ bucket: 'youtube-downloads', objects: [] }) });

    render(<MinioObjectPicker selected={null} onSelect={vi.fn()} />);
    await waitFor(() => expect(fetch).toHaveBeenCalledTimes(1));
    fireEvent.change(screen.getByRole('searchbox'), { target: { value: 'source' } });
    await waitFor(() => expect(fetch).toHaveBeenCalledWith(expect.stringContaining('search=source'), expect.anything()));
  });
});
