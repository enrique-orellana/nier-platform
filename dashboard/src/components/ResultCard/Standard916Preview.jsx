...
export default function Standard916Preview({
    ...
    const cropStyle = useMemo(() => {
        ...
        const left = frameSize.width / 2 - (region.x + region.width / 2) * videoSize.width * scale;
        const top = frameSize.height / 2 - (region.y + region.height / 2) * videoSize.height * scale;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Standard 9:16 gameplay preview">
            <div className="flex max-h-full w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#121214] shadow-2xl">
                ...
                <div className="flex min-h-0 flex-1 flex-col items-center gap-3 overflow-auto p-4">
                    <div ref={frameRef} data-testid="standard-916-preview" data-aspect="9:16" className="relative w-full max-w-[360px] shrink-0 overflow-hidden bg-black aspect-[9/16]">
