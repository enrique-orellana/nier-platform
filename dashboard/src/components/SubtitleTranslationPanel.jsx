...
export default function SubtitleTranslationPanel({
  ...
  const targetAlreadyExists = tracks.some(
    (track) =>
      track.id !== source?.id &&
      track.language?.toLowerCase() === targetLanguage,
  );
  ...
  const translate = async () => {
    ...
    try {
      const response = await fetch(
        getApiUrl(
          `/api/clip/${jobId}/${clipIndex}/versions/${versionId}/subtitle-tracks/translate`,
        ),
        {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            source_track_id: source?.id || "original",
            tracks,
          }),
        },
      );
      ...
      if (!response.ok)
        throw new Error(payload.detail || "Subtitle translation failed.");
      ...
      while (!["done", "error", "failed"].includes(statusPayload.status)) {
        ...
        if (!["done", "error", "failed"].includes(statusPayload.status))
          await sleep(1000);
      }
      ...
      const mergedTracks = [
        ...tracks.filter((track) => track.id !== statusPayload.track?.id),
        statusPayload.track,
      ].filter(Boolean);
  ...
  return (
    <section
      className="rounded-xl border border-white/5 bg-white/[0.02] overflow-hidden flex flex-col shadow-sm"
      aria-label="Subtitle translation"
    >
      <div className="bg-white/[0.03] px-4 py-3 border-b border-white/5 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[10px] text-zinc-500 font-medium uppercase tracking-wider">
            {sourceCueCount} cues • audio intact
          </p>
        </div>
      </div>
      ...
      <div className="p-4 space-y-6">
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs font-semibold text-zinc-500 uppercase tracking-widest">
            ...
            <span className="text-zinc-400">
              {(sourceLanguage || "unknown").toUpperCase()}
            </span>
          </div>
          <SubtitleTrackPicker
            tracks={tracks}
        ...
        <div className="space-y-2">
          <label
            ...
            className="block text-xs font-semibold text-zinc-500 uppercase tracking-widest"
          >
            Target Language
          </label>
          <select
          ...
          >
            {Object.entries(LANGUAGES).map(([code, name]) => {
              const alreadyAdded = tracks.some(
                (track) =>
                  track.id !== source?.id &&
                  track.language?.toLowerCase() === code,
              );
              return (
                <option
                  key={code}
                  value={code}
                  disabled={code === sourceLanguage || alreadyAdded}
