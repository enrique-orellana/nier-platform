...
export default function UGCGallery() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      fetch(getApiUrl("/api/saasshorts/gallery?limit=100")).then((r) =>
        r.ok ? r.json() : { videos: [] },
      ),
      fetch(getApiUrl("/api/saasshorts/actor-gallery")).then((r) =>
        r.ok ? r.json() : { images: [] },
      ),
    ])
      .then(([vData, aData]) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1 flex items-center gap-2">
            ...
            <span className="flex items-center gap-1">
              <User size={14} /> {avatars.length} avatars
            </span>
          </p>
        </div>
        ...
        (videos.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 border border-dashed border-white/5 rounded-3xl bg-white/[0.02]">
            ...
            <p className="text-zinc-600 text-sm mt-1">
              Start by creating your first AI Short
            </p>
          </div>
        ) : (
      ...
      {tab === "avatars" &&
        (avatars.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 border border-dashed border-white/5 rounded-3xl bg-white/[0.02]">
            ...
            <p className="text-zinc-600 text-sm mt-1">
              Generate actors to see them here
            </p>
          </div>
        ) : (
      ...
      <div className="p-3 space-y-2">
        {avatar.description ? (
          <div className="relative group/copy">
            <p className="text-[10px] text-zinc-400 line-clamp-2 leading-relaxed min-h-[2.5em]">
              {avatar.description}
            </p>
            <button
          ...
          <p className="text-[10px] text-zinc-600 italic min-h-[2.5em]">
            No prompt data
          </p>
        )}
  ...
  return (
    <div className="group relative flex flex-col rounded-2xl overflow-hidden bg-zinc-900/50 border border-white/[0.05] hover:border-violet-500/50 hover:bg-zinc-900 transition-all duration-500 hover:-translate-y-1 shadow-xl hover:shadow-violet-500/10">
      ...
      >
        ...
        {/* Overlay */}
        <div
          className={`absolute inset-0 bg-black/40 flex items-center justify-center transition-opacity duration-500 ${playing ? "opacity-0" : "opacity-100"}`}
        >
        ...
        <div className="absolute top-3 left-3">
          <div
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[8px] font-black tracking-tighter backdrop-blur-md border border-white/20 shadow-lg ${
              mode === "lowcost"
      ...
      <div className="p-3 space-y-2.5">
        <div>
          <h3 className="text-xs font-bold text-zinc-100 truncate group-hover:text-violet-400 transition-colors">
            {video.title || "Untitled Video"}
          </h3>
          <p className="text-[10px] text-zinc-500 mt-0.5 flex items-center gap-1.5">
            <span className="font-medium text-zinc-400">
              ${video.cost_estimate?.total?.toFixed(2) || "0.00"}
            </span>
