  ...
  const tracks = editorState.tracks.map((track, index) => ({
    ...
    items: track.items.map((item) => {
      ...
      trackItemsMap[item.id] = {
        ...
        display: {
          from: Math.round(item.start * editorState.fps),
          to: Math.round(item.end * editorState.fps),
        },
        duration: Math.max(
          1,
          Math.round((item.end - item.start) * editorState.fps),
        ),
...
export default function DesignComboTimeline({
  state,
  onStateChange,
  onSelectItem,
  playheadFrame = 0,
  ...
  useEffect(() => {
    ...
    const playheadX =
      ...
      Math.max(0, Math.min(duration, playheadFrame / (state.fps || 30))) *
        pixelsPerSecond;
  ...
  const beginInlineEdit = (event, item) => {
    ...
    setDraftText(item.text || item.label || "");
  };
  const commitInlineEdit = (item) => {
    ...
    const nextItem = {
      ...
      ).map((caption) => ({ ...caption, text: nextText })),
    };
    emitStateChange({
      ...state,
  ...
  const renderItem = (track, item) => {
    ...
    return (
      <div
        ...
        title={label}
        onClick={() => onSelectItem?.(item, track)}
        onDoubleClick={(event) => beginInlineEdit(event, item)}
        onPointerDown={(event) => beginDrag(event, track, item)}
        className={`absolute ${positionClasses} overflow-hidden rounded border border-white/20 px-1 py-1 text-[10px] text-white shadow transition-all`}
        style={{
          left: `${(item.start / duration) * 100}%`,
          width: `${Math.max(0.25, ((item.end - item.start) / duration) * 100)}%`,
          minWidth: 10,
          backgroundColor: colors[track.type] || "#52525b",
          opacity: track.visible === false ? 0.35 : 1,
        }}
      >
        {isEditing ? (
          <input
            autoFocus
            aria-label={`Edit subtitle ${label}`}
            value={draftText}
            onChange={(event) => setDraftText(event.target.value)}
            onBlur={() => commitInlineEdit(item)}
            onKeyDown={(event) => {
              event.stopPropagation();
              if (event.key === "Enter") commitInlineEdit(item);
              if (event.key === "Escape") setEditingItemId(null);
            }}
            onPointerDown={(event) => event.stopPropagation()}
            className="w-full min-w-0 rounded bg-black/30 px-1 text-[10px] text-white outline-none"
          />
        ) : (
          <span
            className={`truncate block ${isCollapsed ? "hidden" : "leading-[14px]"}`}
          >
            {label}
          </span>
        )}
        <span
          role="button"
          tabIndex={0}
          aria-label={`${label} resize start`}
          onPointerDown={(event) => beginDrag(event, track, item, "start")}
          className="absolute left-0 top-0 h-full w-1 cursor-ew-resize opacity-0 hover:opacity-100"
        />
        <span
          role="button"
          tabIndex={0}
          aria-label={`${label} resize end`}
          onPointerDown={(event) => beginDrag(event, track, item, "end")}
          className="absolute right-0 top-0 h-full w-1 cursor-ew-resize opacity-0 hover:opacity-100"
        />
      </div>
    );
  };
  const beginDrag = (event, track, item, edge = "move") => {
    ...
    const update = (moveEvent) => {
      const deltaMs =
        ((moveEvent.clientX - originX) / laneWidth) * duration * 1000;
      const next =
        edge === "move"
          ? moveCue(original, deltaMs, duration * 1000)
          : resizeCue(
              original,
              edge,
              deltaMs,
      ...
      const tracks = state.tracks.map((candidate) =>
        candidate.id === track.id
          ? {
              ...candidate,
              items: candidate.items.map((candidateItem) =>
                candidateItem.id === item.id ? nextItem : candidateItem,
              ),
            }
          : candidate,
      );
  ...
  return (
    <div
      ...
      className="timeline-scroll h-full overflow-auto rounded-lg border border-white/10 bg-[#101014]"
      ref={timelineRef}
    >
      <div
      ...
      >
        <div
          ...
          className="relative ml-40 h-8 border-b border-white/10 bg-[#151519]"
        ...
        >
          <span className="absolute left-1 top-1 text-[10px] text-zinc-600">
            {formatTimecode(0, state.fps || 30)}
          </span>
          <span className="absolute right-1 top-1 text-[10px] text-zinc-600">
            {formatTimecode(duration, state.fps || 30)}
          </span>
        </div>
        {state.tracks.map((track) => {
          ...
          return (
            <div
              key={track.id}
              className={`flex ${isCollapsed ? "h-6" : "h-14"} border-b border-white/10 transition-all overflow-hidden`}
            >
              <TrackControls
                track={track}
              ...
              />
              <div
                className="relative shrink-0 bg-[#111115] transition-all"
                style={{ width: `${laneWidth}px` }}
              >
                {track.items.map((item) => renderItem(track, item))}
              </div>
            </div>
          );
        })}
        <div
          className="absolute top-0 bottom-0 pointer-events-none ml-40"
          style={{ width: `${laneWidth}px` }}
        >
          <div
            className="absolute top-0 bottom-0 w-px bg-red-500 z-50 shadow-[0_0_8px_rgba(239,68,68,0.5)]"
