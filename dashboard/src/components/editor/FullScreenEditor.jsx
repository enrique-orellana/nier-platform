...
export default function FullScreenEditor({ isOpen = true, jobId, clipIndex, clip = {}, initialVersion = null, initialManifest = null, onClose, onRendered }) {
    ...
    const addSubtitleCue = useCallback(() => {
        const subtitleTrack = editorState.tracks.find((track) => track.type === 'subtitle' && track.id === `subtitles-${activeTrackId}`)
        ...
        setEditorState((previous) => ({
            ...previous,
            tracks: previous.tracks.map((track) => track.id === subtitleTrack.id
    ...
    useEffect(() => {
        ...
        window.addEventListener('openshorts:add-subtitle-cue', handleAddSubtitleCue);
        return () => window.removeEventListener('openshorts:add-subtitle-cue', handleAddSubtitleCue);
    }, [addSubtitleCue]);
    ...
    return <div className="fixed inset-0 z-[60] flex flex-col bg-background text-white" data-testid="full-screen-editor"><header className="glass-panel flex h-16 shrink-0 items-center justify-between px-6 m-2 mb-0 z-10"><div className="flex items-center gap-4"><span className="text-lg font-bold tracking-tight text-white drop-shadow-sm">OpenShorts Editor</span><span className="rounded-full bg-primary/20 px-3 py-1 text-xs font-semibold text-primary drop-shadow-sm border border-primary/30">{version ? `Version ${version.version_id.slice(0, 8)}` : 'Draft'}</span></div><button type="button" onClick={onClose} aria-label="close editor" className="rounded-full p-2.5 text-zinc-400 transition-colors hover:bg-white/10 hover:text-white"><X size={20} /></button></header><div className="grid min-h-0 flex-1 grid-cols-[300px_minmax(0,1fr)_320px] grid-rows-[minmax(320px,1fr)_minmax(220px,38vh)] gap-2 p-2"><section className="glass-panel row-span-2 flex flex-col overflow-auto p-5 shadow-lg" aria-label="Media Pool"><h2 className="mb-5 text-xs font-bold uppercase tracking-widest text-primary drop-shadow-sm">Media Pool</h2><MediaPool sourceUrl={inputProps.videoUrl} versions={versions} tracks={subtitleTracks} onSelectVersion={loadVersion} onSelectTrack={setActiveTrackId} /></section><section className="glass-panel min-w-0 overflow-hidden relative shadow-inner border border-white/[0.05] bg-black" aria-label="Viewer"><RemotionPreview videoUrl={inputProps.videoUrl} durationInSeconds={durationSeconds} fps={fps} width={clip.output_width || 1080} height={clip.output_height || 1920} subtitles={inputProps.subtitles} subtitleTracks={inputProps.subtitleTracks} activeSubtitleTrackId={activeTrackId || inputProps.activeSubtitleTrackId} hook={inputProps.hook} effects={inputProps.effects} currentFrame={currentFrame} playing={playing} onFrameChange={setCurrentFrame} onPlayingChange={setPlaying} /></section><aside className="glass-panel row-span-2 flex flex-col overflow-auto p-5 shadow-lg" aria-label="Inspector"><h2 className="mb-5 text-xs font-bold uppercase tracking-widest text-primary drop-shadow-sm">Inspector</h2><InspectorPanel selectedItem={selectedItem} editorState={editorState} activeTrackId={activeTrackId} onTrackChange={setActiveTrackId} onItemChange={updateSelectedItem} translationPanel={translationPanel} /><div className="mt-8 pt-6 border-t border-white/[0.08]"><h3 className="mb-4 text-xs font-bold tracking-widest text-zinc-500 uppercase">Version History</h3><VersionHistory versions={versions} currentVersionId={version?.version_id} selectedVersionId={version?.version_id} onSelect={loadVersion} onBranch={branchVersion} /></div></aside><section className="glass-panel min-w-0 overflow-hidden flex flex-col shadow-inner" aria-label="Timeline"><div className="border-b border-white/[0.08] bg-surfaceLight/40 px-3 py-2"><TransportControls currentFrame={currentFrame} durationFrames={editorState.durationFrames} fps={fps} playing={playing} onPlayingChange={setPlaying} onFrameChange={setCurrentFrame} zoom={zoom} onZoomChange={setZoom} /></div><div className="flex-1 overflow-auto p-4 custom-scrollbar bg-black/20 inset-shadow-sm"><div className="mb-2 flex min-w-[760px] items-center justify-between text-xs text-primary/70 font-semibold tracking-wide"><span>00:00:00</span><span>{durationSeconds.toFixed(2)}s</span></div><DesignComboTimeline state={editorState} onStateChange={setEditorState} onSelectItem={(item, track) => { setActiveTrackId(track.type === 'subtitle' ? track.id.replace(/^subtitles-/, '') : activeTrackId); setSelectedItem({ ...item, trackId: track.id }); }} playheadFrame={currentFrame} onSeek={(seconds) => setCurrentFrame(Math.round(seconds * fps))} zoom={zoom} /></div></section></div><footer className="glass-panel flex h-16 shrink-0 items-center justify-end gap-3 m-2 mt-0 z-10 px-6"><span className="mr-auto max-w-[48%] truncate text-sm font-medium text-red-400" role="alert">{error}</span><button type="button" onClick={onClose} className="rounded-xl px-5 py-2.5 text-sm font-semibold text-zinc-400 transition-colors hover:text-white hover:bg-white/10">Cancel</button><button type="button" onClick={saveVersion} disabled={busy || !version?.version_id} className="btn-primary flex items-center gap-2 drop-shadow-md"><Save size={16} /> {busy ? 'Rendering…' : 'Save as new version'}</button></footer></div>;
}
