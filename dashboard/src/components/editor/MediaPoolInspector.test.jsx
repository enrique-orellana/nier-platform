...
describe('editor media pool and inspectors', () => {
    it('lists source, immutable versions, and subtitle tracks', () => {
        ...
        render(<MediaPool sourceUrl="https://example.test/source.mp4" versions={[{ version_id: 'v2', status: 'completed' }]} tracks={[{ id: 'original', label: 'Original', language: 'es' }]} onSelectVersion={onSelectVersion} onSelectTrack={onSelectTrack} />);
        ...
        expect(onSelectVersion).toHaveBeenCalledWith(expect.objectContaining({ version_id: 'v2' }));
        expect(onSelectTrack).toHaveBeenCalledWith('original');
    });

    it('renders audio controls and emits numeric edits', () => {
        const onChange = vi.fn();
        render(<InspectorPanel selectedItem={{ id: 'audio', type: 'audio', label: 'Source audio', start: 0, end: 10, volume: 1 }} editorState={{ tracks: [] }} onItemChange={onChange} />);
