...
describe("AudioWaveform", () => {
  ...
  it("loads real audio data and renders peak bars", async () => {
    getAudioData.mockResolvedValue({
      durationInSeconds: 4,
      channelWaveforms: [new Float32Array([0, 0.5, 1, 0.25])],
    });
    ...
    render(
      <AudioWaveform videoUrl="blob:demo" durationMs={4000} sampleCount={3} />,
    );

    await waitFor(() =>
      expect(screen.getAllByTestId("audio-waveform-bar")).toHaveLength(3),
    );
    expect(getAudioData).toHaveBeenCalledWith("blob:demo");
    expect(getWaveformPortion).toHaveBeenCalledWith(
  ...
  it("reuses decoded audio data while recalculating bars for a new sample count", async () => {
    ...
    getWaveformPortion.mockImplementation(({ numberOfSamples }) =>
      Array.from({ length: numberOfSamples }, (_, index) => ({
        index,
        amplitude: 0.5,
      })),
    );
    ...
    const { rerender } = render(
      <AudioWaveform videoUrl="blob:cache" durationMs={4000} sampleCount={3} />,
    );
    await waitFor(() => expect(getWaveformPortion).toHaveBeenCalledTimes(1));

    rerender(
      <AudioWaveform videoUrl="blob:cache" durationMs={4000} sampleCount={4} />,
    );

    await waitFor(() => expect(getWaveformPortion).toHaveBeenCalledTimes(2));
    expect(getAudioData).toHaveBeenCalledTimes(1);
    expect(getWaveformPortion).toHaveBeenLastCalledWith(
      expect.objectContaining({ numberOfSamples: 4 }),
    );
  });
  ...
  it("keeps the lane usable when audio decoding fails", async () => {
    ...
    render(
      <AudioWaveform videoUrl="blob:bad" durationMs={4000} sampleCount={3} />,
    );

    await waitFor(() =>
      expect(
        screen.getByText("Audio waveform unavailable"),
      ).toBeInTheDocument(),
    );
