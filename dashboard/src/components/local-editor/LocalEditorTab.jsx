...
function SubtitleInspector({ cue, onChange, onDelete }) {
    ...
    const wordTimings = Array.isArray(cue.captions) ? cue.captions.filter((word) => String(word?.text || '').trim()) : [];
    const createWordTimings = () => onChange({ ...cue, captions: undefined });
    const updateWord = (index, field, value) => {
        const captions = wordTimings.map((word, wordIndex) => wordIndex === index ? { ...word, [field]: field === 'text' ? value : Number(value) } : word);
        onChange({ ...cue, text: captions.map((word) => word.text || '').join(' ').trim(), label: captions.map((word) => word.text || '').join(' ').trim(), captions });
    };
    return (
        <div className="space-y-3">
            ...
            <div className="border-t border-white/10 pt-3">
                <div className="mb-2 flex items-center justify-between">
                    <h4 className="text-xs font-semibold uppercase tracking-[0.14em] text-cyan-200">Word timings</h4>
                    {!wordTimings.length && <button type="button" onClick={createWordTimings} className="rounded-md border border-cyan-300/30 bg-cyan-400/10 px-2 py-1 text-[10px] font-semibold text-cyan-200 hover:bg-cyan-400/20">Create word timings</button>}
                </div>
                {wordTimings.length ? <div className="max-h-56 space-y-1.5 overflow-y-auto rounded-lg border border-cyan-300/15 bg-black/20 p-2">
                    {wordTimings.map((word, index) => <div key={`${word.startMs}-${index}`} className="grid grid-cols-[minmax(0,1fr)_74px_74px] gap-1.5">
                        <input aria-label={`Word ${index + 1} text`} value={word.text || ''} onChange={(event) => updateWord(index, 'text', event.target.value)} className="min-w-0 rounded border border-white/10 bg-black/30 px-2 py-1 text-xs text-white" />
                        <input aria-label={`Word ${index + 1} start`} type="number" value={word.startMs} onChange={(event) => updateWord(index, 'startMs', event.target.value)} className="rounded border border-white/10 bg-black/30 px-2 py-1 text-[11px] text-white" />
                        <input aria-label={`Word ${index + 1} end`} type="number" value={word.endMs} onChange={(event) => updateWord(index, 'endMs', event.target.value)} className="rounded border border-white/10 bg-black/30 px-2 py-1 text-[11px] text-white" />
                    </div>)}
                    <div className="grid grid-cols-[minmax(0,1fr)_74px_74px] gap-1.5 px-2 pt-0.5 text-[9px] uppercase tracking-wider text-zinc-500"><span>Word</span><span>Start</span><span>End</span></div>
                </div> : <p className="text-[11px] leading-5 text-zinc-500">No word timings are attached to this cue yet. Create them to enable word-level highlighting in the preview and export.</p>}
            </div>
        </div>
    );
}
        ...
        <div className="h-full overflow-y-auto bg-[#0d0d0f] text-white">
            ...
            <div className="grid gap-5 p-6 xl:grid-cols-[minmax(0,1fr)_320px]">
                <main className="min-w-0 space-y-5">
                    <div className={clipMetadata ? 'grid items-start gap-4 lg:grid-cols-[220px_minmax(0,360px)] lg:justify-start' : ''}>
