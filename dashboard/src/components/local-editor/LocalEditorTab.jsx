...
function SubtitleStyleInspector({ style, onChange, onRemove, hasCues }) {
    ...
    return (
        <div className="mt-4 space-y-3 border-t border-white/10 pt-4">
            ...
            <button type="button" aria-label={current.bgOpacity > 0 ? 'Hide background box' : 'Show background box'} aria-pressed={current.bgOpacity > 0} onClick={() => update('bgOpacity', current.bgOpacity > 0 ? 0 : 0.5)} className={buttonChoiceClass(current.bgOpacity > 0)}>{current.bgOpacity > 0 ? 'Hide background box' : 'Show background box'}</button>
