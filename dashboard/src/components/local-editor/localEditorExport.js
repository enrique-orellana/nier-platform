...
export const getRecordingOptions = (mimeType, width, height) => {
  ...
  const videoBitsPerSecond = Math.max(
    8_000_000,
    Math.min(24_000_000, Math.round(pixels * 8)),
  );
  return {
    ...(mimeType ? { mimeType } : {}),
    videoBitsPerSecond,
...
export const getExportSourceUrl = (video) =>
...
export const clampOverlayY = (
  ...
  const maxY = Math.max(
    safePadding,
    (Number(canvasHeight) || 0) - (Number(overlayHeight) || 0) + safePadding,
  );
...
export const hookVisualState = (hook = {}, elapsedMs = 0) => {
  ...
  if (hook.entranceAnimation === "fade")
    return { scale, opacity: progress, translateY: 0 };
  if (hook.entranceAnimation === "slide-up")
    return { scale, opacity: progress, translateY: 60 * (1 - progress) };
  if (hook.entranceAnimation === "spring") {
    const springProgress = Math.min(1, Number(elapsedMs) / 350);
    return {
      scale: scale * (0.7 + springProgress * 0.3),
...
export async function renderLocalVideo({
  ...
  if (!video || typeof document === "undefined")
    throw new Error("A local video is required.");
  if (
  ...
  if (!sourceUrl)
    throw new Error("The local video source is not available for export.");
  ...
  try {
    ...
    if (typeof exportVideo.captureStream !== "function")
      throw new Error("This browser cannot capture local video audio.");
    const { width: sourceWidth, height: sourceHeight } =
      getVideoFrameDimensions(exportVideo);
    ...
    try {
      recorder = new MediaRecorder(
        stream,
        getRecordingOptions(mimeType, sourceWidth, sourceHeight),
      );
    } catch {
    ...
    const finish = (error = null) => {
      ...
      if (scheduledFrameId !== null) {
        if (
          ...
          typeof exportVideo.cancelVideoFrameCallback === "function"
        )
          exportVideo.cancelVideoFrameCallback(scheduledFrameId);
    ...
    const draw = () => {
      ...
      if (subtitle) {
        ...
        const subtitleScale =
          subtitleStyleValues.animation === "pop"
            ? 0.9 + subtitleProgress * 0.1
            : 1;
        const subtitleOpacity =
          subtitleStyleValues.animation === "pop" ? subtitleProgress : 1;
        const subtitleColor =
          subtitleStyleValues.animation === "karaoke"
            ? subtitleStyleValues.highlightColor
            : subtitleStyleValues.color;
        const subtitleWidth = canvas.width * 0.88;
        const subtitleFontSize = Math.max(
          24,
          Math.round(
            subtitleStyleValues.fontSize *
              (Math.min(canvas.width, canvas.height) / 440),
          ),
        );
        const subtitleMetrics = measureOverlay(
          context,
          subtitle.text,
          subtitleWidth,
          subtitleFontSize,
        ...
        drawOverlay(context, subtitle.text, {
          ...
          color: subtitleColor,
          background:
            subtitleStyleValues.backgroundOpacity > 0
              ? subtitleStyleValues.background
              : "transparent",
      ...
      const currentHook =
        hook && nowMs >= hook.startMs && nowMs < hook.endMs ? hook : null;
      if (currentHook) {
        const hookState = hookVisualState(
          currentHook,
    ...
    const result = new Promise((resolve, reject) => {
      ...
      recorder.addEventListener("error", () =>
        finish(new Error("Local video export failed.")),
      );
      recorder.start();
      draw();
      exportVideo
        ...
        .catch(() =>
          finish(
            new Error("The browser blocked local video playback for export."),
          ),
        );
    });
