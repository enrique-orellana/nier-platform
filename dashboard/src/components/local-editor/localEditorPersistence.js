...
const openProjectDatabase = () => new Promise((resolve, reject) => {
    if (typeof indexedDB === 'undefined') {
        resolve(null);
        return;
    }
    ...
    request.onupgradeneeded = () => {
        if (!request.result.objectStoreNames.contains(EDITOR_PROJECT_STORE_NAME)) {
            request.result.createObjectStore(EDITOR_PROJECT_STORE_NAME, { keyPath: 'id' });
        }
        if (!request.result.objectStoreNames.contains(EDITOR_PROJECT_VIDEO_STORE_NAME)) {
            request.result.createObjectStore(EDITOR_PROJECT_VIDEO_STORE_NAME, { keyPath: 'projectId' });
        }
    };
    request.onsuccess = () => resolve(request.result);
...
export const normalizeStoredProject = (project = {}) => {
    ...
    return {
        ...
        durationMs: Number.isFinite(Number(project.durationMs)) ? Number(project.durationMs) : 0,
...
const runProjectTransaction = async (mode, callback) => {
    ...
    try {
        return await new Promise((resolve, reject) => {
            const transaction = database.transaction([EDITOR_PROJECT_STORE_NAME, EDITOR_PROJECT_VIDEO_STORE_NAME], mode);
            callback(transaction, resolve, reject);
            transaction.onerror = () => reject(transaction.error || new Error('Could not update local project storage.'));
            transaction.onabort = () => reject(transaction.error || new Error('Could not update local project storage.'));
        });
    } finally {
        database.close();
    }
...
export const saveStoredProject = async (project, file = null) => {
    ...
    const normalized = normalizeStoredProject({ ...project, videoName: file?.name || project?.videoName });
...
export const createStoredProject = async ({ name, history, file, durationMs = 0 }) => saveStoredProject({
    id: randomProjectId(),
    name,
    history,
    videoName: file?.name,
...
export const listStoredProjects = async () => {
    ...
    try {
        ...
        const projects = await new Promise((resolve, reject) => {
            const transaction = database.transaction(EDITOR_PROJECT_STORE_NAME, 'readonly');
            const request = transaction.objectStore(EDITOR_PROJECT_STORE_NAME).getAll();
            request.onsuccess = () => resolve(request.result || []);
            request.onerror = () => reject(request.error || new Error('Could not list local projects.'));
        });
        database.close();
        return projects.map(normalizeStoredProject).sort((a, b) => b.updatedAt - a.updatedAt);
    } catch {
        return [];
    }
};
...
export const loadStoredProject = async (projectId) => {
    ...
    try {
        ...
        const result = await new Promise((resolve, reject) => {
            const transaction = database.transaction([EDITOR_PROJECT_STORE_NAME, EDITOR_PROJECT_VIDEO_STORE_NAME], 'readonly');
            let project;
            let video;
            const projectRequest = transaction.objectStore(EDITOR_PROJECT_STORE_NAME).get(projectId);
            const videoRequest = transaction.objectStore(EDITOR_PROJECT_VIDEO_STORE_NAME).get(projectId);
            projectRequest.onsuccess = () => { project = projectRequest.result || null; };
            videoRequest.onsuccess = () => { video = videoRequest.result || null; };
            transaction.oncomplete = () => resolve(project ? { project: normalizeStoredProject(project), file: fileFromRecord(video) } : null);
            transaction.onerror = () => reject(transaction.error || new Error('Could not load local project.'));
        });
        database.close();
        return result;
    } catch {
...
export const deleteStoredProject = async (projectId) => {
    ...
    try {
        await runProjectTransaction('readwrite', (transaction, resolve) => {
            ...
            transaction.objectStore(EDITOR_PROJECT_VIDEO_STORE_NAME).delete(projectId);
            transaction.oncomplete = resolve;
        });
        if (await getActiveProjectId() === projectId) await setActiveProjectId(null);
        return true;
    } catch {
        return false;
    }
...
export const migrateLegacyProject = async ({ hasLegacyHistory = null } = {}) => {
    try {
        if (localStorage.getItem(EDITOR_PROJECT_MIGRATION_KEY) === 'done') return null;
    } catch {
        // Continue in memory when browser storage is unavailable.
    }

    const historyPresent = hasLegacyHistory === null ? (() => {
    ...
    if (!historyPresent && !file) {
        try { localStorage.setItem(EDITOR_PROJECT_MIGRATION_KEY, 'done'); } catch { /* memory-only fallback */ }
        return null;
    }
