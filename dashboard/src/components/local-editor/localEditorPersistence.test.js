...
const makeVideoFile = (name = 'demo.mp4') => new File(['video'], name, { type: 'video/mp4', lastModified: 123 });
...
describe('local editor project persistence', () => {
    ...
    it('creates, lists, loads, renames, and deletes a stored project', async () => {
        ...
        const project = await createStoredProject({ name: 'Demo Project', history, file, durationMs: 4200 });

        expect(project.name).toBe('Demo Project');
        expect(project.durationMs).toBe(4200);
        expect((await listStoredProjects()).map((item) => item.id)).toEqual([project.id]);
        expect((await loadStoredProject(project.id)).file.name).toBe('demo.mp4');

        await renameStoredProject(project.id, 'Renamed Project');
        expect((await loadStoredProject(project.id)).project.name).toBe('Renamed Project');
