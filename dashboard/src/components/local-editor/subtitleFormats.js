...
const parseTimestamp = (value) => {
  ...
  return (
    ((Number(match[1] || 0) * 60 + minutes) * 60 + seconds) * 1000 +
    Number(match[4])
  );
};
...
const finalize = (cues) => {
  ...
  const normalized = parsed.map((cue) =>
    cue.endMs === cue.startMs ? { ...cue, endMs: cue.startMs + 1 } : cue,
  );
...
const parseCueBlock = (block, index, preserveId = false) => {
  ...
  return {
    id: preserveId
      ? lines[timingIndex - 1]?.trim() || `subtitle-${index + 1}`
      : `subtitle-${index + 1}`,
    text,
...
export function parseSrt(source) {
  return finalize(
    splitBlocks(source).map((block, index) => parseCueBlock(block, index)),
  );
}
  ...
  return finalize(
    body
      ? splitBlocks(body).map((block, index) =>
          parseCueBlock(block, index, true),
        )
      : [],
  );
}
