...
import { requiresAiApiKey, resolveAiBaseUrl, shouldForwardApiKey } from './aiProvider';

describe('AI provider URL resolution', () => {
    it('does not reuse a saved local endpoint for OpenRouter', () => {
        expect(resolveAiBaseUrl('openrouter', 'http://host.docker.internal:1234'))
            .toBe('https://openrouter.ai/api/v1');
    });

    it('preserves the configured endpoint for local providers', () => {
        expect(resolveAiBaseUrl('lmstudio', 'http://host.docker.internal:1234'))
