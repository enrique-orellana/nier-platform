...
export const normalizeCodexModels = (payload = {}) => {
  ...
  const normalizeEfforts = (model) => {
    const rawEfforts = model?.supported_reasoning_levels
    ...
    return rawEfforts.map((rawEffort) => {
      const effort = typeof rawEffort === 'string'
        ? rawEffort
        : rawEffort?.effort || rawEffort?.reasoning_effort || rawEffort?.reasoningEffort
          || rawEffort?.id || rawEffort?.value || rawEffort?.name;
      const id = String(effort || '').trim().toLowerCase();
      if (!id || effortIds.has(id)) return null;
      effortIds.add(id);
      return {
        id,
        label: String(rawEffort?.label || rawEffort?.title || rawEffort?.displayName || formatEffortLabel(id)).trim(),
        description: String(rawEffort?.description || rawEffort?.details || '').trim(),
      };
    }).filter(Boolean);
  };
...
export const pickCodexEffort = ({ currentEffort, modelId, models = [] }) => {
  const normalized = String(currentEffort || '').trim().toLowerCase();
  if (!normalized || normalized === 'auto' || normalized === 'default') return 'auto';
  const model = models.find((entry) => entry.id === modelId);
  if (!model || !Array.isArray(model.efforts) || model.efforts.length === 0) return 'auto';
  return model.efforts.some((effort) => effort.id === normalized) ? normalized : 'auto';
};
