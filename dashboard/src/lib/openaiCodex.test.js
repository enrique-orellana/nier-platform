...
describe("OpenAI Codex helpers", () => {
  ...
  it("labels pending, reconnect, connected, and disconnected states", () => {
    ...
    expect(codexStatusLabel({ requiresReconnect: true })).toBe(
      "Reconnect ChatGPT",
    );
  ...
  it("requires reconnect after a terminal poll error", () => {
    expect(
      codexPollState({ status: "expired", connected: false, pending: false }),
    ).toEqual({
      connected: false,
      pending: false,
  ...
  it("keeps an available model and falls back to Auto when it disappears", () => {
    ...
    expect(pickCodexModel({ currentModel: "retired-model", models })).toBe(
      "auto",
    );
  });
});
