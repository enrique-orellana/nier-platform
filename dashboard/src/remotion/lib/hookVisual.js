...
export const getHookPositionStyle = (position = 'top', layoutFormat = 'standard', facecamSize = 'medium') => {
...
export const getHookBoxStyle = (hook = {}, renderWidth = HOOK_PREVIEW_WIDTH) => {
    ...
    return {
        ...
        boxShadow: isStreamer ? 'none' : '0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -4px rgba(0, 0, 0, 0.1)',
        WebkitTextStroke: isStreamer ? `${Math.max(2, Math.round(2 * scale))}px #000000` : undefined,
        textShadow: isStreamer ? '1px 1px 0 #000000, -1px -1px 0 #000000, 1px -1px 0 #000000, -1px 1px 0 #000000' : undefined,
        textAlign: 'center',
    };
};
