...
export const FACECAM_HEIGHT_RATIOS = {
    small: 0.3,
    medium: 0.38,
    large: 0.46,
};
...
export const getHookFontSize = (fontSize = 48, size = "M", renderWidth = HOOK_PREVIEW_WIDTH) => Math.max(14, ((Number(fontSize) || 48) / 2.6) *
    (HOOK_SIZE_SCALE[size] || HOOK_SIZE_SCALE.M)) * widthScale(renderWidth);
export const getStreamerBoundaryRatio = (facecamSize = "medium") => FACECAM_HEIGHT_RATIOS[facecamSize] || FACECAM_HEIGHT_RATIOS.medium;
...
export const getHookPositionCoordinates = (hook = {}, renderWidth = HOOK_OUTPUT_WIDTH, renderHeight = HOOK_OUTPUT_HEIGHT) => {
...
export const getHookPositionStyle = (positionOrHook = "top", layoutFormat = "standard", facecamSize = "medium", renderWidth = HOOK_OUTPUT_WIDTH, renderHeight = HOOK_OUTPUT_HEIGHT) => {
    const hook = typeof positionOrHook === "string"
...
export const getHookCardBorderRadius = (boxStyle = "rounded", index = 0, lineCount = 1, renderWidth = HOOK_PREVIEW_WIDTH) => {
    if (normalizeHookBoxStyle(boxStyle) !== "headline_cards")
        return undefined;
...
export const getHookCardOverlap = (boxStyle = "rounded", renderWidth = HOOK_PREVIEW_WIDTH) => normalizeHookBoxStyle(boxStyle) === "headline_cards"
