...
def test_local_editor_render_uploads_source_and_starts_backend_render(monkeypatch, tmp_path):
    ...
    body = FakeRenderClient.request_body
    assert body["props"]["videoUrl"].startswith(f"/videos/{payload['jobId']}/")
    stored_source = tmp_path / payload["jobId"] / "source.mp4"
    assert stored_source.read_bytes() == b"video-bytes"
